...
public class InventoryClickListener extends LobbyListener {
  ...
  private Inventory createMapSelectionInventory(
      HumanEntity player, Collection<JumpyJumpMap> mapSet) {
    ...
    List<Integer> slots =
        Arrays.stream(IntStream.rangeClosed(0, maps.size() - 1).toArray())
            .boxed()
  ...
  private void animateLoading(
      Player player, List<JumpyJumpMap> maps, Inventory inventory, List<Integer> slots) {
    ...
    final ItemStack mapItem =
        new ItemStackBuilder(Material.EMPTY_MAP).setName("§e" + map.getName()).create();
