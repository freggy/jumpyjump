...
public class ConfigDeserializer implements JsonDeserializer<Config> {
  ...
  public Config deserialize(
    ...
    ItemStack challengeItem =
        ItemStackUtil.createItemStackFromJson(object.get("challenge-item").getAsJsonObject());
    ItemStack quickJoin =
        ItemStackUtil.createItemStackFromJson(object.get("quick-join-item").getAsJsonObject());
    ItemStack lobby =
        ItemStackUtil.createItemStackFromJson(object.get("lobby-item").getAsJsonObject());
    ItemStack quickJoinLeave =
        ItemStackUtil.createItemStackFromJson(
            object.get("quick-join-leave-item").getAsJsonObject());

    NPC quickJoinNpc = this.npcFromJson(object.get("quick-join-npc").getAsJsonObject());
    Location statsNpcLocation =
        LocationUtil.locationFromJson(object.get("stats-npc-location").getAsJsonObject());

    return new Config(
        challengeItem, lobby, quickJoin, quickJoinLeave, statsNpcLocation, quickJoinNpc);
  }
