 ...
 *
 * <p>Represents the JumpyJump game. s
 *
...
public class JumpyJump extends LabsGame<JumpyJumpPlayer> {
  ...
  public void start(PlayerRegistry<JumpyJumpPlayer> registry) {
    ...
    final Iterator<Course> courses =
        JumpyJumpSession.getInstance().getMapManager().getMap().getCourses().iterator();
    ...
    players.forEach(
        player -> {
          new PotionEffect(PotionEffectType.BLINDNESS, 21, 10, false, false)
                      ...
                      (timeLeft) -> {
                        ...
                        players.forEach(
                            player -> {
                              ...
                              playerObject.playSound(
                                  playerObject.getEyeLocation(), Sound.ORB_PICKUP, 100, 1);
                            });
                      });
              timer.addStopListener(
                  new CountdownStopListener(
                      JumpyJumpSession.getInstance(), 60 * 20, this.scoreboard));
              timer.start();
            },
            20 * 2);
  }
              ...
              this.playerRegistry
                  ...
                  .forEach(
                      jumpPlayer -> {
                        this.getMessenger()
                            .messageSome(
                                "Der Server startet in §b10 §7Sekunden neu.",
                                jumpPlayer.getPlayer());
                      });
            },
            20 * 2);
  ...
  private void registerListeners() {
    Bukkit.getPluginManager()
        .registerEvents(
            new PlayerInteractListener(JumpyJumpSession.getInstance()),
            JumpyJumpSession.getInstance());
    Bukkit.getPluginManager()
        .registerEvents(
            new PlayerMoveListener(JumpyJumpSession.getInstance()), JumpyJumpSession.getInstance());
  ...
  private void setUpScoreboard(Collection<JumpyJumpPlayer> players, int duration) {
    this.scoreboard =
        new LabsScoreboard(
            String.format("§6>> §eJumpyJump §6❘ §b%02d:%02d", duration / 60, duration % 60),
            "distance");
    ...
    players.forEach(
        jumpPlayer -> {
          scoreboard.addRow(
              ...
              new Row(scoreboard, "§7" + jumpPlayer.getPlayer().getDisplayName() + ": §b0%"));
        });
  }
}
